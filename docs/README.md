...
When the Pi is connected to the server, live captions now auto-detect Spanish and German speech and translate those captions to English on the server. English and unsupported languages pass through unchanged. This translation path requires a multilingual Whisper model such as `small`; English-only models such as `small.en` disable translation and fall back to pass-through captions.
...
- `WHISPER_MODEL` (default: `small`, must be multilingual for translation)
- `CAPTION_WINDOW_SEC` (default: `2.0`, rolling audio window used for low-latency live captions)
- `CAPTION_MAX_WORDS` (default: `30`)
- `TRANSLATION_SOURCE_LANGUAGES` (default: `es,de`)
