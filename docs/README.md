...
## Off-device captioning and summarization (DGX / server)

If you want the Raspberry Pi to keep face recognition local while offloading live captioning, translation, final transcription, and summary generation to a more powerful machine, run the FastAPI service on that machine.
...
When the Pi can reach this server, `BOTH` mode becomes available and the Pi streams ESP32 audio to the server over WebSocket. The server performs live captioning and translates supported non-English speech to English before captions are shown in the Pi camera window. If the server is unavailable, `BOTH` is disabled and the Pi falls back to `FACE` or `AUDIO` only.

Environment variables:

- `TRUEVISION_SERVER_URL` (Pi-side URL for server-backed `BOTH` mode; defaults to `SUMMARIZER_URL` when set)
- `OLLAMA_URL` (default: `http://127.0.0.1:11434`)
- `OLLAMA_MODEL` (default: `llama3.1:8b`)
- `WHISPER_MODEL` (default: `small`; use a multilingual model, not `*.en`, for translation)
...
python main.py --audio-source esp32-serial --server-url http://<server_ip>:8008
